...
function omit_if(race,who)--冲突判断
    if(race==1)then--BOSS
        for if_i=1,4,1 do--单阵营最大人数(4人)
            if(if_i~=who and (boss[who]==boss[if_i] or boss[who]==spy[if_i] or boss[who]==hero[if_i]))then
                return false
            end
        end
    elseif(race==2)then--HERO
        for if_i=1,4,1 do--单阵营最大人数(4人)
            if(if_i~=who and (hero[who]==boss[if_i] or hero[who]==spy[if_i] or hero[who]==hero[if_i]))then
                return false
            end
        end
    elseif(race==3)then--SPY
        for if_i=1,4,1 do--单阵营最大人数(4人)
            if(if_i~=who and (spy[who]==boss[if_i] or spy[who]==spy[if_i] or spy[who]==hero[if_i]))then
                return false
            end
        end
    end
