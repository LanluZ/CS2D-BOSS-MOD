...
function start_r()
    ...
    -------------------------------------------
        ...
        if(boss[1]~=0)then
            choose[1]=math.random(1,2)--选择随机的boss
            if(choose[1]==1)then--1号boss——苏维埃最后的战甲
                image("gfx/SkinsMenu/super-tank.png", 1, 1, 200 + boss[1])--给他贴图
                parse('setmaxhealth '..boss[1]..' '..250)
                parse('sethealth '..boss[1]..' '..250)
                parse('equip '..boss[1]..' 81')--装甲
                parse('equip '..boss[1]..' 48')--炮火
                parse('equip '..boss[1]..' 90')--机炮
                parse('speedmod '..boss[1]..' -5')--移动速度减缓
                msg2(boss[1],'\169000000255你是苏维埃最后的战甲@C')
                msg2(boss[1],'\169000000255你拥有强大的火力和坚固的装甲@C')
                msg2(boss[1],'\169000000255现在，夺回我们的人质！@C')
                fix[1]=8
            end

            if(choose[1]==2)then--2号boss——日冕
                image("gfx/SkinsMenu/sun-king.png", 1, 1, 200 + boss[1])--给他贴图
                parse('setmaxhealth '..boss[1]..' '..250)
                parse('sethealth '..boss[1]..' '..250)
                parse('equip '..boss[1]..' 84')--隐身
                parse('equip '..boss[1]..' 45')--激光
                parse('speedmod '..boss[1]..' 20')--高速
                msg2(boss[1],'\169000000255你是日冕@C')
                msg2(boss[1],'\169000000255你拥有强大的机动力和毁灭性的光耀@C')
                msg2(boss[1],'\169000000255现在，夺回我们的人质！@C')
                fix[1]=2
            end
        end
        ...
        if(boss[2]~=0)then--2号boss分配
            ...
            if(choose[2]==1)then--1号boss——苏维埃最后的战甲
                image("gfx/SkinsMenu/super-tank.png", 1, 1, 200 + boss[2])--给他贴图
                parse('setmaxhealth '..boss[2]..' '..250)
                parse('sethealth '..boss[2]..' '..250)
                parse('equip '..boss[2]..' 81')--装甲
                parse('equip '..boss[2]..' 48')--炮火
                parse('equip '..boss[2]..' 90')--机炮
                parse('speedmod '..boss[2]..' -5')--移动速度减缓
                msg2(boss[2],'\169000000255你是苏维埃最后的战甲@C')
                msg2(boss[2],'\169000000255你拥有强大的火力和坚固的装甲@C')
                msg2(boss[2],'\169000000255现在，夺回我们的人质！@C')
                fix[2]=8
            end

            if(choose[2]==2)then--2号boss——日冕
                image("gfx/SkinsMenu/sun-king.png", 1, 1, 200 + boss[2])--给他贴图
                parse('setmaxhealth '..boss[2]..' '..250)
                parse('sethealth '..boss[2]..' '..250)
                parse('equip '..boss[2]..' 84')--隐身
                parse('equip '..boss[2]..' 45')--激光
                parse('speedmod '..boss[2]..' 20')--高速
                msg2(boss[2],'\169000000255你是日冕@C')
                msg2(boss[2],'\169000000255你拥有强大的机动力和毁灭性的光耀@C')
                msg2(boss[2],'\169000000255现在，夺回我们的人质！@C')
                fix[2]=2
            end
        end
...
function end_r()
    ...
    if(num~=0)then
        --1号BOSS抽取
        if(num>=2)then
            boss[1]=player_num[math.random(1,num)]--boss抽取
            while(player(boss[1],"exists")==false)do--超高校级的防止boss为空
                boss[1]=player_num[math.random(1,num)]
            end
        end

        --1号英雄抽取
        if(num>=3)then
            hero[1]=player_num[math.random(1,num)]
            while(player(hero[1],"exists")==false or hero[1]==boss[1])do--超高校级的防止英雄为空
                hero[1]=player_num[math.random(1,num)]
            end
        end
        ...
        --2号英雄抽取
        if(num>=4)then
            hero[2]=player_num[math.random(1,num)]
            while(player(hero[2],"exists")==false or hero[2]==boss[1])do--超高校级的防止英雄为空
                hero[2]=player_num[math.random(1,num)]
            end
            while(hero[2]==hero[1])do
                hero[2]=player_num[math.random(1,num)]
            end
        end
        ...
        --3号英雄抽取
        if(num>=5)then
            hero[3]=player_num[math.random(1,num)]
            while(player(hero[3],"exists")==false or hero[3]==boss[1])do--超高校级的防止英雄为空
                hero[3]=player_num[math.random(1,num)]
            end
            while(hero[3]==hero[1] or hero[3]==hero[2])do
                hero[3]=player_num[math.random(1,num)]
            end
        end
        ...
        --4号英雄抽取
        if(num>=6)then
            hero[4]=player_num[math.random(1,num)]
            while(player(hero[4],"exists")==false or hero[4]==boss[1])do--超高校级的防止英雄为空
                hero[4]=player_num[math.random(1,num)]
            end
            while(hero[4]==hero[1] or hero[4]==hero[2] or hero[3]==hero[4])do
                hero[4]=player_num[math.random(1,num)]
            end
        end
        ...
        --2号BOSS抽取
        if(num>=7)then
            boss[2]=player_num[math.random(1,num)]
            while(player(boss[2],"exists")==false or boss[2]==boss[1])do--超高校级的防止boss为空
                boss[2]=player_num[math.random(1,num)]
            end
            while(boss[2]==hero[1] or boss[2]==hero[2] or boss[2]==hero[3] or boss[2]==hero[4])do
                boss[2]=player_num[math.random(1,num)]
            end
        end
        ...
        --1号CT抽取
        if(num>=8)then
            spy[1]=player_num[math.random(1,num)]--超高校级的防止spy为空
            while(spy[1]==boss[1] or spy[1]==boss[2])do
                spy[1]=player_num[math.random(1,num)]
            end
            while(spy[1]==hero[1] or spy[1]==hero[2] or spy[1]==hero[3] or spy[1]==hero[4])do
                spy[1]=player_num[math.random(1,num)]
            end
        end
