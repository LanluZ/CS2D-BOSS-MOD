...
-------------------------------------------
    ...
    if(num~=0)then
        ...
        if(spy[1]~=0)then
            ...
            msg2(spy[1],'\169000000255你是爆破警察，你身上有无穷无尽的燃烧弹@C')
        end
...
local function fix_boss(boss_num)--填入boss编号而非id
    if(fix[boss_num]~=nil and choose[1]~=nil and id==boss[boss_num] and action==1)then
        if(fix[boss_num]>=1 and choose[1]==1)then
            ...
            msg2(boss[boss_num],'\169255000000启动紧急维修！剩余:'..fix[boss_num]..'次@C')
            msg(player(boss[boss_num],"name").."所在机甲已启动修理，剩余"..fix[boss_num].."次!@C")
